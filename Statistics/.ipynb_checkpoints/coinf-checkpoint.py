...
vec = np.vectorize(np.int64)
k2 = vec(k1)

for i in (k2):    
    out_arc[i] = np.arcsin(i)
    i+=1
...
ax2 = ax1.twinx()
ax2.bar(k1, out_arc, color = 'pink', alpha = 0.3,edgecolor='None')
ax1.bar(k1, distr,color = 'b', alpha = 0.8,edgecolor='None')
plt.show()
